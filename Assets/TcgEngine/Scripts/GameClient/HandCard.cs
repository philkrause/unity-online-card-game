...
namespace TcgEngine.Client
{
    ...
    public class HandCard : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (IsFocus())
            {
                target_position = deck_position + Vector2.up * 160f;
                BringToFront();

                // Update card position and size as needed
                // transform.position = Vector2.Lerp(transform.position, target_position, Time.deltaTime * focus_speed);
                // transform.localScale = Vector3.Lerp(transform.localScale, target_size, Time.deltaTime * focus_speed);
            }
